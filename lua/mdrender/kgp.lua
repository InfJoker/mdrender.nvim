--- Kitty graphics protocol — Unicode placeholder helpers.
---
--- The Unicode-placeholder variant of the kitty graphics protocol displays an
--- image by writing placeholder cells (U+10EEEE) into the *text grid*. Each cell
--- carries combining diacritics encoding its row/column within the image, and a
--- foreground color encoding the image id. Because the image is anchored to real
--- text cells, it survives Neovim redraws and works through tmux passthrough —
--- unlike absolute placement.
local M = {}

M.PLACEHOLDER = 0x10EEEE

-- The 297 row/column diacritics, derived from kitty's rowcolumn-diacritics.txt.
-- Index i (1-based) encodes row/column value (i-1).
-- stylua: ignore
M.DIACRITICS = {
...
--- Encode a Unicode codepoint as UTF-8 (LuaJIT has no utf8 library).
function M.utf8(cp)
  if cp < 0x80 then
...
local function diacritic(i)
  -- i is 0-based row/col value
  local cp = M.DIACRITICS[i + 1] or M.DIACRITICS[#M.DIACRITICS]
  if not DIA[cp] then
    DIA[cp] = M.utf8(cp)
  end
  return DIA[cp]
...
--- Maximum row/column index supported by the diacritic table.
M.MAX_CELLS = #M.DIACRITICS

--- A single placeholder cell for image (row, col), both 0-based.
function M.cell(row, col)
  PH = PH or M.utf8(M.PLACEHOLDER)
  return PH .. diacritic(row) .. diacritic(col)
end
